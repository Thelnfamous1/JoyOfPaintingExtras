...
public class CustomCanvasType {
    ...
    public static final String CANVAS_SCALE_TAG_KEY = "CanvasScale";
    ...
    public static float getCustomScale(ItemStack stack) {
        CompoundTag tag = stack.getTag();
        return getCustomScale(tag);
    }

    public static float getCustomScale(@Nullable CompoundTag tag) {
        if(tag != null && tag.contains(CANVAS_SCALE_TAG_KEY)){
            return tag.getFloat(CANVAS_SCALE_TAG_KEY);
        }
        return 0.75F;
    }

    public static void setCustomScale(ItemStack stack, float scale){
        stack.getOrCreateTag().putFloat(CANVAS_SCALE_TAG_KEY, scale);
    }

    public static float calculateScaleFactor(float scale, int width, int height){
        return 16.0F * (scale / Math.max(width, height));
    }
}